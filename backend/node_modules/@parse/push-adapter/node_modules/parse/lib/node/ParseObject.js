  ...
  save(target, options) {
    ...
    if (Array.isArray(target)) {
      ...
      if (options && options.transaction && pending.length > 1) {
        if (pending.some(el => !(0, _canBeSerialized.default)(el))) return Promise.reject(new _ParseError.default(_ParseError.default.OTHER_CAUSE, 'Tried to save a transactional batch containing an object with unserializable attributes.'));
